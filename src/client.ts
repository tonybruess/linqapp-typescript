...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['LINQAPP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['LINQAPP_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Linqapp API.
 */
export class Linqapp {
  apiKey: string;
   ...
   * API Client for interfacing with the Linqapp API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['LINQAPP_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['LINQAPP_BASE_URL'] ?? https://api.linqapp.com/api/partner] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('LINQAPP_BASE_URL'),
    apiKey = readEnv('LINQAPP_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.LinqappError(
        "The LINQAPP_API_KEY environment variable is missing or empty; either provide it, or instantiate the Linqapp client with an apiKey option, like new Linqapp({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Linqapp.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('LINQAPP_LOG'), "process.env['LINQAPP_LOG']", this) ??
      defaultLogLevel;
  ...
  static LinqappError = Errors.LinqappError;
...
Linqapp.Chats = Chats;
Linqapp.Messages = Messages;
Linqapp.Attachments = Attachments;
Linqapp.Phonenumbers = Phonenumbers;
Linqapp.WebhookEvents = WebhookEvents;
Linqapp.WebhookSubscriptions = WebhookSubscriptions;

export declare namespace Linqapp {
